...
// It adds some helper methods along with Meta to the original CustomPluginDefinition object.
type CustomPluginDefinition struct {
	kong.CustomPluginDefinition `yaml:",inline"`
	Meta
}

// Identifier returns the entity name or ID.
func (c1 *CustomPluginDefinition) Identifier() string {
	if c1.Name != nil {
		return *c1.Name
	}
	if c1.ID != nil {
...
// Console returns an entity's identity in a human readable string.
func (c1 *CustomPluginDefinition) Console() string {
	return c1.Identifier()
}

// Equal returns true if c1 and c2 are equal.
func (c1 *CustomPluginDefinition) Equal(c2 *CustomPluginDefinition) bool {
	return c1.EqualWithOpts(c2, false, false)
}
...
// If ignoreID is set to true, IDs will be ignored while comparison.
// If ignoreTS is set to true, timestamp fields will be ignored.
func (c1 *CustomPluginDefinition) EqualWithOpts(c2 *CustomPluginDefinition, ignoreID, ignoreTS bool) bool {
	c1Copy := c1.DeepCopy()
	c2Copy := c2.DeepCopy()
