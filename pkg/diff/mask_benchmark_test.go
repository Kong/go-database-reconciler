...
-----END PRIVATE KEY-----"`)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			baselineResult := maskEnvVarValueBaseline(diffData)
			if !strings.Contains(baselineResult, maskedValue) {
				t.Fatalf("baseline masking produced no %q output", maskedValue)
			}
			cache := NewEnvVarCache()
			_ = maskEnvVarValueWithCache(diffData, cache)
			...
			var finalBaselineResult string
			for range 2 {
				finalBaselineResult = maskEnvVarValueBaseline(diffData)
			}
			baselineDuration := time.Since(baselineStart)
			baselineAverage := baselineDuration / 2
			_ = finalBaselineResult // Verify result was computed
