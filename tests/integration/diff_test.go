...
`
   ...
   "config": {
     ...
     "cache_tokens": true,
     "cache_tokens_salt": "5CjRldqVE1mQtsunhqKGAAA4U3big9Xc",
     "cache_ttl": 3600,
     "cache_ttl_max": null,
     "cache_ttl_min": null,
     "cache_ttl_neg": null,
...
Summary:
  ...
  Deleted: 0
`
	expectedOutputPluginUpdateChangedNewFieldsOpenIdConnect312x = `updating plugin openid-connect (global)  {
   "config": {
     "anonymous": null,
     "audience": null,
     "audience_claim": [
...
Summary:
  ...
  Deleted: 0
`
	expectedOutputPluginUpdateChangedNewFieldsRLA312x = `updating plugin rate-limiting-advanced (global)  {
   "config": {
     "compound_identifier": null,
     "consumer_groups": null,
     "dictionary_name": "kong_rate_limiting_counters",
...
Summary:
  ...
  Deleted: 0
`
	expectedOutputPluginUpdateChangedOldFieldsOpenIdConnect312x = `updating plugin openid-connect (global)  {
   "config": {
     "anonymous": null,
     "audience": null,
     "audience_claim": [
...
Summary:
  ...
  Deleted: 0
`
	expectedOutputPluginUpdateChangedOldFieldsRLA312x = `updating plugin rate-limiting-advanced (global)  {
   "config": {
     "compound_identifier": null,
     "consumer_groups": null,
     "dictionary_name": "kong_rate_limiting_counters",
...
func Test_Diff_PluginUpdate_NewerThan312x(t *testing.T) {
	...
	tests := []struct {
		...
		// Thus, we are doing one plugin check at a time to avoid failures due to ordering.
		{
			...
			initialStateFile: "testdata/diff/004-plugin-update/initial-ee-openid.yaml",
			stateFile:        "testdata/diff/004-plugin-update/kong-ee-changed-new-fields-openid.yaml",
			expectedDiff:     expectedOutputPluginUpdateChangedNewFieldsOpenIdConnect312x,
		},
		{
			name:             "initial vs updating by sending only old fields with new values - plugin rate-limiting-advanced",
			initialStateFile: "testdata/diff/004-plugin-update/initial-ee-rla.yaml",
			stateFile:        "testdata/diff/004-plugin-update/kong-ee-changed-new-fields-rla.yaml",
			expectedDiff:     expectedOutputPluginUpdateChangedNewFieldsRLA312x,
		},
		{
			name:             "initial vs updating by sending only new fields with new values - plugin openid-connect",
			initialStateFile: "testdata/diff/004-plugin-update/initial-ee-openid.yaml",
			stateFile:        "testdata/diff/004-plugin-update/kong-ee-changed-old-fields-openid.yaml",
			expectedDiff:     expectedOutputPluginUpdateChangedOldFieldsOpenIdConnect312x,
		},
		{
			name:             "initial vs updating by sending only new fields with new values - plugin rate-limiting-advanced",
			initialStateFile: "testdata/diff/004-plugin-update/initial-ee-rla.yaml",
			stateFile:        "testdata/diff/004-plugin-update/kong-ee-changed-old-fields-rla.yaml",
			expectedDiff:     expectedOutputPluginUpdateChangedOldFieldsRLA312x,
		},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
...
func Test_Diff_PluginConfig_Nested_Arrays(t *testing.T) {
	...
	t.Skip(
		"TODO: https://github.com/Kong/go-database-reconciler/issues/379 - " +
			"skipping: jwt-signer nested array normalization changed in Kong 3.12, " +
			"causing reordered elements to produce an unexpected diff",
	)
	client, err := getTestClient()
	require.NoError(t, err)
...
func Test_Diff_PluginConfigReorderArraySetValues(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			initialStateFile: "testdata/diff/004-plugin-update/initial-request-transformer-reorder-array.yaml",
